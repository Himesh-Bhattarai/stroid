/**
 * @module tests/coverage-extra.test
 *
 * LAYER: Tests
...
import { clearAllStores as clearAllStoresInternal } from "../src/store-write.js";
import { initialStates } from "../src/store-lifecycle/registry.js";
import { beginTransaction, endTransaction, isTransactionActive } from "../src/store-transaction.js";
import {
  parsePath,
  validateDepth,
  getByPath,
...
test("computed guards invalid arguments and missing entries", async () => {
  resetAllStoresForTest();
  const { createComputed, invalidateComputed, deleteComputed } = await import("../src/computed.js");
